...
function getTabs(dict: any) {
  return [
    { key: "general", label: dict.tabs.general, icon: <Info className="h-3.5 w-3.5" /> },
    { key: "lots", label: dict.tabs.lots, icon: <Package className="h-3.5 w-3.5" /> },
    { key: "documents", label: dict.tabs.documents, icon: <FileText className="h-3.5 w-3.5" /> },
    { key: "soumission", label: dict.tabs.soumission, icon: <Send className="h-3.5 w-3.5" /> },
  ] as const;
}
  ...
  return (
    <div className="space-y-4">
      <dl className="divide-y divide-slate-100">
        <InfoRow label={dict.general.reference} value={<span className="font-mono font-bold text-[#364150]">{ao.reference}</span>} />
        ...
        />
        {ao.estimatedAmount && (
          <InfoRow label={dict.general.estimatedAmount} value={<span className="font-semibold text-[#364150]">{ao.estimatedAmount}</span>} />
        )}
        <InfoRow
          ...
          value={
            <span className="flex items-center gap-1.5 text-rose-600 font-semibold">
              ...
              {new Date(ao.deadline).toLocaleDateString(locale === "ar" ? "ar-DZ" : "fr-DZ", {
                weekday: "long",
                day: "numeric",
                month: "long",
                year: "numeric",
...
function SoumissionTab({ ao, dict, locale }: { ao: OeAoItem; dict: any; locale: string }) {
  if (ao.hasSubmission && ao.submissionStatus) {
    const statusLabel = dict.soumission.statusLabels;
    ...
    return (
      <div className="space-y-4">
        ...
        <div className={`flex items-center gap-3 rounded-xl border p-4 ${statusColor[ao.submissionStatus]}`}>
          ...
          <div>
            <p className="text-sm font-bold">{dict.soumission.banner.title}</p>
            <p className="text-[11px]">
              {dict.soumission.banner.currentStatus}{" "}
        ...
        <dl className="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white px-4">
          <InfoRow label={dict.soumission.summary.reference} value={ao.reference} />
          <InfoRow label={dict.soumission.summary.lots} value={`${ao.lots.length} ${dict.soumission.summary.lotsCount}`} />
          <InfoRow label={dict.soumission.summary.date} value={
          ...
          <InfoRow label={dict.soumission.summary.hash} value={<span className="font-mono text-[10px] break-all text-slate-500">a3f9b2c1d8e4f7…</span>} />
        </dl>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {dict.soumission.actions.download}
          </button>
        </div>
      </div>
    );
    ...
    <div className="space-y-4">
      {isExpired ? (
        <div className="flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-700">
          ...
          <div>
            <p className="text-sm font-bold">{dict.soumission.notSubmitted.expiredTitle}</p>
            <p className="text-[11px]">{dict.soumission.notSubmitted.expiredDesc}</p>
          </div>
        </div>
      ) : (
        <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-blue-700">
          ...
          <div>
            <p className="text-sm font-bold">{dict.soumission.notSubmitted.noSubTitle}</p>
            <p className="text-[11px]">
              {dict.soumission.notSubmitted.noSubDesc}
              <br />
              {dict.soumission.notSubmitted.deadline}{" "}
              <span className="font-semibold">
                {deadline.toLocaleDateString(locale === "ar" ? "ar-DZ" : "fr-DZ", { day: "numeric", month: "long", year: "numeric" })}
              </span>
            </p>
          </div>
        </div>
      ...
      <div>
        <h3 className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">
          {dict.soumission.checklist.title}
        </h3>
        <ul className="space-y-1.5">
          {dict.soumission.checklist.items.map((doc: string) => (
...
export default function OeAoDetailPage({ aoId, dict, locale }: { aoId: string; dict: any; locale: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <nav className="flex items-center gap-1.5 text-[11px] text-slate-500">
        ...
        >
          {dict.page.breadcrumb.dashboard}
        </button>
      ...
      <header className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center gap-2">
          ...
          >
            <ArrowLeft className={`h-3.5 w-3.5 ${locale === "ar" ? "rotate-180" : ""}`} />
            {dict.page.back}
          </button>
        </div>
