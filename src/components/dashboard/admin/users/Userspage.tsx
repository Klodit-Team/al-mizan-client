...
export default function UsersPage({ locale, dict }: UsersPageProps) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch profiles + available roles in parallel
      const [profiles, roleList] = await Promise.all([
        getAdminUserProfiles(),
        getAdminRoles().catch(() => [] as RoleEntity[]),
      ]);

      setRoles(roleList);

      // Build a roleId → roleName lookup from the fetched role list
      const roleIdToName = Object.fromEntries(
        roleList.map((r) => [r.id, r.name])
      ) as Record<string, User["role"]>;

      // For each profile fetch the user's current role assignment
      const usersWithRoles = await Promise.all(
        profiles.map(async (profile) => {
          let roleName: User["role"] = "OPERATEUR_ECONOMIQUE";
          let roleId = "";

          try {
            const assignments = await getUserRoles(profile.userId);
            if (assignments.length > 0) {
              // Use the most recent assignment
              const latest = assignments[assignments.length - 1];
              roleId = latest.roleId;
              roleName = roleIdToName[latest.roleId] ?? roleName;
            }
          } catch {
            // Role fetch failed — keep defaults
          }
          ...
          const mapped: User = {
            ...
            email: "", // ProfileEntity has no email; populated by auth if needed
            role: roleName,
            roleId,
  ...
  const handleRoleChange = async (userId: string, targetRoleName: string) => {
    // Resolve the selected role name to a UUID
    const targetRole = roles.find((r) => r.name === targetRoleName);
    if (!targetRole) {
      console.warn("Role UUID not found for:", targetRoleName);
      return;
    }
    ...
    setUsers((prev) =>
      prev.map((u) =>
        u.userId === userId
          ? { ...u, role: targetRoleName as User["role"], roleId: targetRole.id }
          : u
      )
    );

    try {
      // Send correct AssignRoleDto: { userId (UUID), roleId (UUID) }
      await assignUserRole({ userId, roleId: targetRole.id });
    } catch (error) {
  ...
  const handleBlacklistSubmit = async () => {
    ...
    // user.id is the OE record ID; user.userId is the auth user UUID
    ...
    try {
      // API expects { reason } (not motif) and the OE record id in path
  ...
  // oeId = OE record id (not auth userId)
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-sm font-bold text-gray-700">
            {dict.listTitle} ({filtered.length})
          </h2>
          {isLoading && (
                      ...
                      <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          <select
                            ...
                            className="text-xs font-semibold px-2 py-1 rounded-md border border-gray-200 bg-white text-gray-700 outline-none focus:border-blue-500 cursor-pointer disabled:opacity-50"
                          >
                            {Object.entries(roleLabels).map(([key, label]) => (
                        ...
                        <td className="px-5 py-3">
                          ...
                          >
                            {user.is_blacklisted
                              ? dict.statusLabels.blacklisted
                              : user.is_active
                              ? dict.statusLabels.active
                              : dict.statusLabels.inactive}
                          </span>
                        </td>

                        {/* Created at — from ProfileEntity.createdAt */}
                        ...
                        <td className="px-5 py-3 text-right">
                          {user.role === "OPERATEUR_ECONOMIQUE" &&
                            (!user.is_blacklisted ? (
                              <button
                                onClick={() =>
                                  setBlacklistModal({ isOpen: true, user, motif: "" })
                                }
