...
export default function Sidebar({ locale, role, dict }: SidebarProps) {
  ...
  // Routes qui ne doivent matcher qu'exactement (évite le faux-positif startsWith)
  const EXACT_ONLY = [
    `/${locale}/dashboard/commission`,
    `/${locale}/dashboard/commission/mes-commissions`,
  ];

  const isActive = (href: string) => {
    if (EXACT_ONLY.includes(href)) return pathname === href;
          ...
          >
            <span
              className={
                isActive(item.href)
                  ? "text-[#4CAF50]"
                  : "text-gray-400 group-hover:text-[#2F9E44] transition-colors duration-150"
              }
            >
              {item.icon}
            </span>
